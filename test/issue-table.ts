...
async function gradeTs(): Promise<Row[] | null> {
  ...
  return tsTests.map(t => ({ id: idOf(t.label), title: t.label.replace(/^#[\w./-]+:\s*/, ''), mono: pass(mono, t), off: pass(off, t) }));
}
...
const langs: { name: string; opponent: string; rows: Row[] | null }[] = [
  { name: 'TypeScript', opponent: 'microsoft/TypeScript-TmLanguage', rows: await gradeTs() },
  { name: 'HTML', opponent: "VS Code's html.tmLanguage", rows: await gradeHtml() },
  { name: 'Vue', opponent: 'vuejs/language-tools vue.tmLanguage.json', rows: await gradeVue() },
];

// ── render ──
const mark = (b: boolean) => b ? '✓' : '·';
const graded = langs.filter(l => l.rows) as { name: string; opponent: string; rows: Row[] }[];
...
// summary table — the at-a-glance ✓ overview
md += '| language | vs hand-written grammar | Monogram | official | only Monogram | only official |\n';
md += '|---|---|:--:|:--:|:--:|:--:|\n';
for (const { name, opponent, rows } of graded) {
  const om = rows.filter(r => r.mono && !r.off).length, oo = rows.filter(r => !r.mono && r.off).length;
  md += `| ${name} | ${opponent} | **${rows.filter(r => r.mono).length}/${rows.length}** | ${rows.filter(r => r.off).length}/${rows.length} | ${om} | ${oo} |\n`;
}
for (const l of langs) if (!l.rows) md += `| ${l.name} | — | _skipped (no official grammar)_ |  |  |  |\n`;
// per-language detail — small grammars get the full ✓ table; large ones get the asymmetry lists
for (const { name, rows } of graded) {
  const om = rows.filter(r => r.mono && !r.off), oo = rows.filter(r => !r.mono && r.off), nn = rows.filter(r => !r.mono && !r.off), bb = rows.filter(r => r.mono && r.off);
  const tag = (r: Row) => r.mono && !r.off ? ' **(only Monogram)**' : (!r.mono && r.off ? ' **(only official)**' : (!r.mono && !r.off ? ' _(both miss)_' : ''));
  md += `\n#### ${name}\n`;
  if (rows.length <= 15) {
    md += '| issue | Monogram | official |\n|---|:--:|:--:|\n';
    for (const r of [...om, ...oo, ...nn, ...bb]) md += `| ${r.id} — ${r.title}${tag(r)} | ${mark(r.mono)} | ${mark(r.off)} |\n`;
  } else {
    // compact: dedupe ids, show the #-bearing tracker issues, fold coverage-only cases into "+N more"
    const fmt = (arr: Row[]) => { const ids = [...new Set(arr.map(r => r.id))]; const real = ids.filter(id => /#\d/.test(id)); const extra = ids.length - real.length; return (real.join(', ') || (extra ? '' : '—')) + (extra ? `${real.length ? ' · ' : ''}+${extra} more` : ''); };
    md += `**Only Monogram solves (${om.length}):** ${fmt(om)}\n\n`;
    md += `**Only official solves (${oo.length}):** ${fmt(oo)}\n\n`;
    md += `**Both miss (${nn.length}, shared TM ceilings):** ${fmt(nn)} · both solve: ${bb.length}\n`;
  }
}
