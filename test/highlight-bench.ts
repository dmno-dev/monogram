...
// The headline: a direct Monogram-vs-official comparison, both highlighter engines, as
// a table. Token-family accuracy = did the highlighter put each token in the right
// family (type / value / keyword / …), graded vs a neutral tsc oracle. The TextMate row
// is computed here; the tree-sitter row is HARDCODED (this lightweight readme job can't
// build the wasm) but gated separately by `npm run gate:treesitter`. JS/TSX → footnote.
function buildBenchMarkdown(rows: LangRow[], jsx?: { exact: number; family: number; graded: number }): string {
  ...
  const extra: string[] = [];
  if (js) extra.push(`JavaScript **${rowPct(js.mono)}** (vs ${rowPct(js.off)})`);
  if (jsx && jsx.graded > 0) extra.push(`TSX matches official **${(jsx.family / jsx.graded * 100).toFixed(0)}%** family`);
  return [
    ...
    '| --- | --- | --- |',
    `| TextMate | **${rowPct(ts.mono)}** | ${rowPct(ts.off)} |`,
    '| tree-sitter | **95.9%** | 92.7% |',
    '',
    `<sub>TypeScript, on the ambiguity-rich documented-bug ledger ([\`test/issue-cases.ts\`](test/issue-cases.ts)) where regex grammars slip${extra.length ? '. ' + extra.join('; ') : ''}. tree-sitter is CI-gated (\`npm run gate:treesitter\`); regenerate: \`npm run bench:readme\`.</sub>`,
  ].join('\n');
}
