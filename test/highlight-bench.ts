...
// A deliberately compact block: the headline TypeScript number as one sentence, then
// the other targets folded into a single footnote. Token-family accuracy = did the
// highlighter put each token in the right family (type / value / keyword / …), graded
// vs a neutral tsc oracle. tree-sitter is hardcoded (bench:readme can't build wasm; it
// is gated separately by `npm run gate:treesitter`).
function buildBenchMarkdown(rows: LangRow[], jsx?: { exact: number; family: number; graded: number }): string {
  const ts = rows.find((r) => r.lang === 'TypeScript') ?? rows[0];
  const js = rows.find((r) => r.lang === 'JavaScript');
  const others: string[] = [];
  if (js) others.push(`JavaScript **${rowPct(js.mono)}** (vs ${rowPct(js.off)})`);
  others.push('tree-sitter **95.9%** (vs 92.7%, CI-gated)');
  if (jsx && jsx.graded > 0) others.push(`TSX matches official **${(jsx.family / jsx.graded * 100).toFixed(0)}%** family`);
  return [
    '<!-- generated by `npm run bench:readme` — do not edit by hand -->',
    '',
    `**Highlighter — token-family accuracy vs a neutral \`tsc\` oracle:** Monogram (derived) **${rowPct(ts.mono)}** vs the official hand-written grammar **${rowPct(ts.off)}** for TypeScript, on the ambiguity-rich documented-bug ledger ([\`test/issue-cases.ts\`](test/issue-cases.ts)) where regex grammars slip.`,
    '',
    `<sub>Same grammar, other targets: ${others.join(' · ')}. Regenerate: \`npm run bench:readme\`.</sub>`,
  ].join('\n');
}
