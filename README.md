...
One conformance-proven grammar — **a third the official's hand-written size**, on a language-agnostic engine — yet **more correct than the official highlighter** (below); JavaScript and the JSX dialects derive from the same definition. A few scope differences are [deliberate](#known-differences-from-the-official-highlighter).

<!-- bench:start -->
<!-- generated by `npm run bench:readme` — do not edit by hand -->

**Highlighter — token-family accuracy vs a neutral `tsc` oracle:** Monogram (derived) **87.6%** vs the official hand-written grammar **76.2%** for TypeScript, on the ambiguity-rich documented-bug ledger ([`test/issue-cases.ts`](test/issue-cases.ts)) where regex grammars slip.

<sub>Same grammar, other targets: JavaScript **92.6%** (vs 90.7%) · tree-sitter **95.9%** (vs 92.7%, CI-gated) · TSX matches official **100%** family. Regenerate: `npm run bench:readme`.</sub>
<!-- bench:end -->
