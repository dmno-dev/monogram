...
**Highlighter correctness, per language — Monogram vs the official grammar.** Auto-generated by [`test/highlight-bench.ts`](test/highlight-bench.ts); TypeScript and JavaScript are graded by a neutral `tsc` oracle, TSX by agreement with the official grammar (tsc has no JSX oracle):
