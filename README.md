...
Those figures are *derived* from a parser proven at **100%** valid-code coverage and **97.8%** bidirectional agreement with `tsc` — from one **~1,050-line** grammar (JS + TS) that replaces the official **3331-line** hand-written TextMate, on a language-agnostic engine. A few scope differences are [deliberate](#known-differences-from-the-official-highlighter).
